...
- JPA 구현체들은 객체 그래프를 마음껏 탐색할 수 있도록 지원하는데, 프록시 기술을 사용
- 객체를 조회할 때 연관된 객체를 즉시 로딩하는 방법을 즉시 로딩, 연관된 객체를 지연해서  
로딩하는 것을 지연 로딩이라 함
  - 조인 전략
    - @ManyToOne, @OneToOne
    ...
    - @OneToMany, @ManyToMany
      ...
      - (optional = true) : 외부 조인
- 객체를 저장하거나 삭제할 때 연관된 객체도 함께 저장하거나 삭제할 수 있는데 이것을 영속성 전이
...
- Value Type 특징
  - 식별자가 없음
  - 생명 주기를 엔티티에 의존
    - 스스로 생명주기를 가지지 않고 엔티티에 의존(제거 포함)
  - 공유하지 않는 것이 안전함
    - 값을 복사해서 사용해야 함
    - 오직 하나의 주인만이 관리해야함
    - 불면(immutable) 객체로 만드는 것이 안전  
